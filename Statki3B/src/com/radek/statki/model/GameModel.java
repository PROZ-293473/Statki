...
import com.radek.statki.atj.ConsumerB;
import com.radek.statki.atj.ProducerB;
...
//	public Board opponentBoard = gamerBoard;
	...
	public void sendGamerBoard() {
		ProducerB producer = null;
		try {
			producer = new ProducerB("localhost:7676/jms", "ATJ2Queue");
			...
			System.out.println("B wyslalem staki");
	...
	public void sendShot(int x, int y) {
		...
		try {
			...
			System.out.println("B wyslalem strzal" + String.valueOf(x) + " " + String.valueOf(y));
		} catch (JMSException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} finally {
	...
	public void receiveOpponentBoard() {
		ConsumerB consumer = null;
		try {

			consumer = new ConsumerB("localhost:7676/jms", "ATJQueue", this);
			consumer.receiveQueueMessageAsync();
			createOpponentShips(lastMessage);
			System.out.println("B odebra³em:" + lastMessage);
			lastMessage = "";
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		...
		try {
			...
			if (game == State.turnP) {
				if (checkShotOpponent(Character.getNumericValue(lastMessage.charAt(0)),
					...
					game = State.turnG;
					System.out.println("B:dosta³em nie poprawny cios" + lastMessage.charAt(0) + lastMessage.charAt(2));
					System.out.println(lastMessage);
					return false;
				} else {
					game = State.turnP;
					System.out.println("B:dosta³em poprawny cios" + lastMessage.charAt(0) + lastMessage.charAt(2));
					System.out.println(lastMessage);
					return false;
				}
			}
